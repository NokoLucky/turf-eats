...
function OrderItems({ orderId }: { orderId: string }) {
    ...
    return (
        <ul className="space-y-4">
            {items.map((item) => (
                <li key={item.id} className="flex flex-col gap-1 bg-background p-3 rounded-xl border shadow-sm">
                    ...
                    {item.selectedOptions && Object.keys(item.selectedOptions).length > 0 && (
                      <div className="flex flex-wrap gap-1 mt-1">
                        {Object.entries(item.selectedOptions).map(([group, choices]) => (
                           choices.map(choice => (
                             <Badge key={`${group}-${choice}`} variant="secondary" className="text-[9px] h-4 font-bold bg-muted text-muted-foreground">
                               {group}: {choice}
                             </Badge>
                           ))
                        ))}
    ...
    <div className="container py-12 px-4 sm:px-8">
        ...
        <Card className="border-none shadow-premium rounded-[2.5rem] overflow-hidden bg-card">
                    ...
                    <TableRow key={order.id} className="hover:bg-muted/30 transition-colors">
                      <TableCell className="align-top pl-8 py-6">
                          ...
                          {order.notes && (
                              <div className="mt-4 p-3 bg-primary/5 rounded-2xl border border-primary/10">
                                  <p className="text-[10px] text-primary uppercase font-black tracking-widest mb-1">Customer Request</p>
                                  <p className="text-xs italic text-foreground/70">"{order.notes}"</p>
                              </div>
                          )}
                      </TableCell>
                      <TableCell className="align-top py-6 font-medium text-muted-foreground">
                      ...
                      <TableCell className="text-right align-top pr-8 py-6">
                       <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="rounded-2xl border-none shadow-2xl p-2 min-w-[180px] bg-popover">
            ...
            {!areOrdersLoading && (!orders || orders.length === 0) && (
              <div className='text-center py-24 text-muted-foreground bg-muted/10'>
